.plot_bivariate <- function(out, title=title, xlab=xlab, ylab=ylab, fan,
  ...
  if(fan){
    ...
    label.x.pos <- .transform_radial(tibble(x = seq(0,1,1/4), y = 1), yoff = 0.08)
    ...
    grid.draw(textGrob(label.x, 
                       label.x.pos$x, label.x.pos$y, gp=gpar(cex=0.75)))
    ...
    grid.draw(textGrob("uncertainty", 0.1, 0.1, rot=300, gp=gpar(cex=0.75)))
    popViewport()
    ...
    pushViewport(viewport(x=0.84, y=0.8,
                          width=0.2, height=0.02))
    grid.draw(textGrob("standard deviation", 0.1, 0.1, rot=90, gp=gpar(cex=0.75)))
    popViewport()
