...
.bivariate_colour_scheme <- matrix(ncol=2, c(
  "1-1", "#402d76",
  "2-1", "#6b588f",
  "3-1", "#9283aa",
  "4-1", "#b8b1c3",
...
.plot_bivariate <- function(out, title=title, xlab=xlab, ylab=ylab, x, y){
  
  na_ind <- which(is.na(out[, y]))
  if(length(na_ind)>0){
    out[na_ind, y] <- 0
  }
  out$bi_class <- bi_class(out, x = x, y = y)
  if(length(na_ind)>0){
  ...
  g2 <- .legend_builder(out, colours=.bivariate_colour_scheme[,2], x, y)
...
.legend_builder <- function(out, colours=.bivariate_colour_scheme[,2], x,
  ...
  ggplot(dat_leg, aes(x=x, y=y)) + geom_tile(stat = "identity", 
    ...
    scale_x_continuous(limits = c(breaks_x[1], breaks_x[5]), expand = c(0, 0),
                       breaks=round(breaks_x,3)) +
