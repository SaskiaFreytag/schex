...
Illumina NextSeq 500. This data is handly availabe in the [`SeuratData` package](https://github.com/satijalab/seurat-data).
Note that we will convert the data from a `Seurat object` to a `SingleCellExperiment`
in order to be able to run a Bioconductor workflow.

```{r load}
InstallData("pbmc3k")
tenx_pbmc3k <- pbmc3k

tenx_pbmc3k <- as.SingleCellExperiment(tenx_pbmc3k)
```
