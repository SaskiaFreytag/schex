...
a subsetted dataset of Peripheral Blood Mononuclear Cells (PBMC) freely available from 
10x Genomics. There are 80 single cells with 230 features in this dataset.
...
The dataset already contains two dimension reductions (PCA and TSNE). We
will now add UMAP. Since there is a random 
...
specifies that I divide my x range into 10 bins. Note that this might be a 
...
```{r calc-hexbin}
pbmc_small <- make_hexbin(pbmc_small, nbins = 10, 
...
plot_hexbin_meta(pbmc_small, col="nCount_RNA", action="median")
```

#### Plot gene expression in hexagon cell representation

Finally, I will visualize the gene expression of the CD1C gene in the 
hexagon cell representation.

```{r plot-gene, fig.height=7, fig.width=7}
gene_id <-"CD1C"
schex::plot_hexbin_feature(pbmc_small, type="RNA", feature=gene_id, 
...
```{r}
gene_id <-"CD1C"
gg <- schex::plot_hexbin_feature(pbmc_small, type="RNA", feature=gene_id, 
    action="mean", xlab="UMAP1", ylab="UMAP2", 
    title=paste0("Mean of ", gene_id))
gg + theme_void()
```
